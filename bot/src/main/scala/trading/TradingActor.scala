...
class TradingActor(context: ActorContext[BotMessage]) extends AbstractBehavior[BotMessage](context) {
  ...
  override def onMessage(message: BotMessage): Behavior[BotMessage] =
    message match
      case SetAPIKeysMessage(apiKeyFromMessage: String, apiSecretFromMessage: String) =>
      ...
      case OpenPositionMessage(strategyName: String, openLongPosition: Boolean, startClosePrice: Double) =>
        ...
        val order: Order = Order("BTC", openLongPosition, quantity, startClosePrice)
        context.log.info(s"Opening position for strategy:$strategyName with isLong:$openLongPosition, quantity:$quantity")
        bybitApi.openPosition(order, apiKey, apiSecret)
        activePositions = activePositions + (strategyName -> order)

      case ClosePositionMessage(strategyName: String) =>
        if !activePositions.contains(strategyName) then
          context.log.error(s"No position is currently active for strategy:$strategyName")
        else
          context.log.info(s"Closing position for strategy:$strategyName on Bybit")
          val orderToClose: Order = activePositions(strategyName)

          bybitApi.closePosition(orderToClose, apiKey, apiSecret)
