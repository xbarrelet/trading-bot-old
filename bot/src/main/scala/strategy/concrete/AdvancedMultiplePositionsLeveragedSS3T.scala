...
class AdvancedMultiplePositionsLeveragedSS3T(val signal: Signal, override val leverage: Int) extends AdvancedStrategy {
  ...
  override def addQuote(quote: Quote): Unit = 
    ...
    if !hasEntered then
      ...
      if !hasOpenPosition && entryPriceReached && isPriceAbove(close, signal.firstTargetPrice) then
        buyInSenseOfSignal()
        hasOpenPosition = true
        firstTargetReached = true
        currentEntryPrice = close

      if hasOpenPosition && firstTargetReached && isPriceBelow(close, signal.firstTargetPrice) then
        shouldExitTradeBool = true
        hasOpenPosition = false
        firstTargetReached = false
        currentEntryPrice = 0.0

      if !hasOpenPosition && firstTargetReached && isPriceAbove(close, signal.secondTargetPrice) then
        buyInSenseOfSignal()
        hasOpenPosition = true
        secondTargetReached = true
        currentEntryPrice = close

      if hasOpenPosition && secondTargetReached && isPriceBelow(close, signal.secondTargetPrice) then
        shouldExitTradeBool = true
        hasOpenPosition = false
        secondTargetReached = false
        currentEntryPrice = 0.0

      if isPriceAbove(close, signal.thirdTargetPrice) then
        shouldExitBool = true
