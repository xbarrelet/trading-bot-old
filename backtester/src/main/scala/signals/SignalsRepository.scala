...
object SignalsRepository {
  ...
  private def cacheSignalsOlderThanOneMonth(startTimestamp: Double): List[Signal] =
    sql"select entry_price, first_target_price, second_target_price, third_target_price, is_long, stop_loss, symbol, timestamp from signals where signal_id = 332" //BTC 1 Year fake signal
//    sql"select entry_price, first_target_price, second_target_price, third_target_price, is_long, stop_loss, symbol, timestamp from signals where signal_id in (327, 328)" //BTC signal
//    sql"select entry_price, first_target_price, second_target_price, third_target_price, is_long, stop_loss, symbol, timestamp from signals where timestamp < ${(System.currentTimeMillis / 1000) - 2419200} and timestamp > $startTimestamp  and signal_id < 331 order by timestamp desc"
      .query[Signal]
      .to[List]
      .transact(transactor)
      .unsafeRunSync()
