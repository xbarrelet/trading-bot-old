...
class SimpleStrategyBacktesterActor(context: ActorContext[Message]) extends AbstractBehavior[Message](context) {
  ...
  override def onMessage(message: Message): Behavior[Message] =
    message match
      case BacktestStrategyMessage(strategy: SimpleStrategy, symbol: String, minutesPerQuote: Int, replyTo: ActorRef[Message]) =>
        var tradesNotEnteredCounter = 0

        val quotes: List[Quote] = quotesRepository.getQuotes(symbol, minutesPerQuote)
